...
def pulsesinc(k,n):
    ...
    for kx in k:
        for nx in n:
            ...
            p += ((1-np.abs(f))**2)*((np.sinc((kx-nx)*(1-np.abs(f)))))/len(k)
    ...
    p = 20*np.log10(p/np.max(p)+0.00000001)
    return p,pm,f
...
def pulsegauss(k,n,a):
    ...
    #p = p/np.sum(p)
    p = 20*np.log10(p/np.max(p)+0.00000001)
    pm = 20*np.log10(pm/np.max(pm)+0.00000001)
    return p,pm,f
...
plt.plot(f,psd_add,'--', color = color_list[2], label=r'Added signal')
...
plt.plot(f,psdmx, color= color_list[2], label=r'Added signal')
plt.xlabel(r'normalized Frequency $fT$')
plt.ylabel(r'$R(f)$ (dB)')
plt.grid('--')
plt.ylim(-50,10)
