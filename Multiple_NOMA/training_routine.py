...
def Multipl_NOMA(M=4,sigma_n=0.1,train_params=[50,300,0.005],canc_method='none', modradius=1, plotting=True, encoder=None):
    """ 
    Training process of NN autoencoder for multiplicative NOMA
    ...
    encoder : already trained encoders can be put in front of encoders that still need training
    """
...
#optimizer.add_param_group({"params": h_est})
    ...
    if plotting==True:
        if device=='cpu':
            ...
            plot_training(validation_SERs.cpu().detach().numpy(), validation_received,cvalid,M, cp.asnumpy(constellations), gmi, decision_region_evolution, cp.asnumpy(meshgrid), constellation_base,gmi_exact.detach().cpu().numpy(),gmi_est2.detach().cpu().numpy()) 
    if canc_method=='nn':
        if device=='cpu':
