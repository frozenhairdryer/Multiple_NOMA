...
sigma, gmi, snr_all = pickle.load( open( "sigmasweep.pkl", "rb" ) )

plt.figure("GMI sigma sweep",figsize=(6,2.5))
for num in range(runs):
    plt.scatter(cp.asnumpy(sigma[:,0])**2,gmi[:,num],color=color_list[0],alpha=0.5)
...
plt.tight_layout()
plt.legend(loc=3)
plt.savefig("GMI_noisesweep_fix.pgf")
