...
#optimizer.add_param_group({"params": h_est})
        ...
        for step in range(int(batches_per_epoch)):
            ...
            for num in range(len(M)):        
                ...
                if num==0:
                    ...
                    modulated = enc[0](batch_labels_onehot).to(device)
                    # Propagate through channel 1; 0.5*sigma because noise is added to inphase and quadrature
                    received = torch.add(modulated, 0.5*sigma_n[num]*torch.randn(len(modulated),2).to(device))
                else:
                    modulated = enc[num](batch_labels_onehot).to(device)
                ...
                if num==len(M)-1 and canc_method=='nn':
                    ...
                    for dnum in range(len(M)):
                        ...
                        if dnum==0:
                            ...
                            decoded_valid[dnum]=dec[dnum](cancelled)
                            cancelled = (canc[dnum](cancelled[dnum-1],enc[dnum](y_valid_onehot)))
    ...
    if plotting==True:
        ...
        constellation_base = []
        for num in range(len(M)):
            t = (torch.view_as_complex(enc_best[num](torch.eye(int(M[num]), device=device)))).cpu().detach().numpy()
