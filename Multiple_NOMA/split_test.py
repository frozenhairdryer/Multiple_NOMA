...
if choice == 4:
    ...
    M=torch.tensor([4,4], dtype=int)
    sigma_n=torch.tensor([0.18,0.18], dtype=float)
    waveform_Multipl_NOMA(M,sigma_n,train_params=cp.array([60,600,0.004]),canc_method='div', modradius=torch.tensor([1,1/3*np.sqrt(2)],device=device), plotting=True, chrom_disp=True)
